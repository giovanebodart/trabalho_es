...
BENCHMARK_NAMES := scale_allocations fire_test tree
...
$(BUILD_DIR)/test_old_pages$(EXEEXT): tests/test_old_pages.c \
		...
		src/old_pages.h src/allocator.h include/interval_tree.h \
		tests/test.h | $(BUILD_DIR)
	$(CC) $(CFLAGS) -Iinclude -Isrc -Itests tests/test_old_pages.c \
